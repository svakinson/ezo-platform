        ...
        <div className="relative p-8 pt-10">
          ...
          <div className="space-y-3 mb-6 pb-6 border-b border-white/10">
            <div className="flex items-center gap-3 text-sm">
              ...
              <span className="text-slate-300 font-semibold leading-snug">{plan.apartments}</span>
            </div>
            <div className="flex items-center gap-3 text-sm">
              <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center">
              ...
              <span className="text-slate-300 font-semibold leading-snug">{plan.buildings}</span>
            </div>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10 bg-white/5">
                    <th className="text-left p-6 text-slate-300 font-semibold leading-snug">ფუნქცია</th>
                    {plans.map(plan => (
                    ...
                    <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      {row.values.map((val, j) => (
                        <td key={j} className="p-6 text-center">
                          {typeof val === 'boolean' ? (
                            val ? (
                              <div className="inline-flex w-8 h-8 rounded-full bg-emerald-500/20 items-center justify-center mx-auto">
                                <IconCheck className="w-5 h-5 text-emerald-400" />
                              </div>
                            ) : (
                              <div className="inline-flex w-8 h-8 rounded-full bg-white/5 items-center justify-center mx-auto">
                                <IconX className="w-5 h-5 text-slate-600" />
                              </div>
                            )
                          ) : (
                            <span className="text-slate-300 font-medium leading-snug">{val}</span>
                          )}
                        </td>
                      ))}
                    </tr>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="bg-slate-900/50 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:border-white/20 transition-colors group">
                ...
                <p className="text-sm text-slate-400 leading-relaxed pl-10">{faq.answer}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="relative bg-slate-900/80 backdrop-blur-md border border-white/10 rounded-3xl p-12 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a 
                ...
                className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white text-slate-900 font-semibold rounded-xl hover:bg-slate-200 transition-all hover:scale-105 leading-snug"
              >
                📧 info@ezo.ge
              </a>
              <a 
