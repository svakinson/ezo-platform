  ...
  return (
    <div className="min-h-screen bg-slate-950 relative overflow-hidden">
      ...
      <header className={`sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl border-b border-white/10 ${viewAsUser ? 'top-[41px]' : ''}`}>
        ...
        {/* ⃝ ზედა ხაზი: რი ლამაზი ბანერი */}
        <div className="border-b border-white/5 bg-slate-900/30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between gap-3">
            
            {/* ბანერი 1: პაკეტის სტატუსი */}
            <div className="flex items-center gap-2 sm:gap-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-3 sm:px-4 py-2 flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm min-w-0">
                <span className="text-slate-300 font-medium whitespace-nowrap">თქვენი მიმდინარე პაკეტია:</span>
                <span className="text-emerald-400 font-bold whitespace-nowrap">🎁 საცდელი</span>
                <span className="hidden sm:inline text-slate-500">•</span>
                <span className="hidden sm:inline text-slate-400 whitespace-nowrap">დარჩენილია:</span>
                <span className="text-amber-400 font-bold whitespace-nowrap">6 დღე</span>
              </div>
            </div>

            {/* ბანერი 2: პაკეტების შეძენა ღილაკი */}
            <Link 
              href="/pricing"
              className="group flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white text-xs sm:text-sm font-bold px-4 sm:px-5 py-2 rounded-xl transition-all hover:-translate-y-0.5 shadow-lg shadow-emerald-500/25 whitespace-nowrap flex-shrink-0"
            >
